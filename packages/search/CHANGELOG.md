...
- 6945ac2: Initial public release: extracted from gnars-website/mcp-subgraph and generalized for any Nouns Builder DAO on Base. Adds plugin registry for CLI/MCP tool extension, per-DAO database isolation for semantic search, and hybrid DAO config (CLI flag > env).

### Patch Changes

- dbbe55c: Audit fixes across both packages.
  ...
  - A failed fetch no longer advances the sync watermark (which permanently skipped
    ...
    watermark tracks the newest _fully synced_ proposal rather than wall-clock time:
...
- Updated dependencies [dbbe55c]
- Updated dependencies [6945ac2]
  - @builder-dao/cli@1.0.0
