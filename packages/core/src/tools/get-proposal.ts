...
export async function getProposal(input: GetProposalInput, ctx: RunContext): Promise<GetProposalOutput | null> {
  let proposal: SubgraphProposal | null = null;

  if (typeof input.id === "number") {
    proposal = await ctx.subgraph.fetchProposalByNumber(input.id);
  } else if (input.id.startsWith("0x")) {
    proposal = await ctx.subgraph.fetchProposalById(input.id);
  } else {
    // Try parsing as number
    const num = parseInt(input.id, 10);
    if (!isNaN(num)) {
      proposal = await ctx.subgraph.fetchProposalByNumber(num);
    }
  }
